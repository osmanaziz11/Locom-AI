...
const RequestCard = ({
  ...
  const email = `${firstName}src/example.com`;
  switch (type) {
    case 'active':
      return (
        <View style={[styles.container, {height: 88, padding: 20}]}>
          ...
          <Button
            style={{height: 42, width: 108, marginLeft: 'auto'}}
            onPress={onPress}>
            Decide
          </Button>
        </View>
      ...
      return (
        <View style={[styles.container, {height: 80}]}>
          ...
          <View style={{flex: 1}}>
            ...
            <AppText numberOfLines={1} color="#8083A3" weight={500}>
              {email}
            </AppText>
          </View>
          <View
            style={[
              styles.label,
              {backgroundColor: isApproved ? '#F2FFF6' : '#FFF6F6'},
            ]}>
            <AppText color={isApproved ? '#27D858' : '#E74E4E'}>
