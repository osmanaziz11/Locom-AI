...
import { AppInput, AppText, Button } from 'src/components/ui';
...
const MemberCreateScreen = ({navigation}) => {
  ...
  return (
    <View
      ...
      <AppText color="#8083A3" weight={400} size={14}>
        Fill the fields below to add new team member
      </AppText>
      <ScrollView>
          ...
          {members.map((member, i) => {
            return (
              <Pressable
                ...
                onPress={handlePressMember(member, i)}>
                <AppText
                  weight={700}
                  size={12}
                  color="#fff">{`${member.firstname} ${member.lastname}`}</AppText>
                <ChevronIcon
                  color="#fff"
      ...
      <Button onPress={handleProceed}>Proceed to pay</Button>
    </View>
  );
};
