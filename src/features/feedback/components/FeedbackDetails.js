...
const FeedbackDetails = ({initialData}) => {
  ...
  const renderComments = ({item}) => {
    return <ReviewCard {...item} text={item.message} />;
  };
  ...
  return (
    <View style={styles.container}>
      <ProfileImage
        size={105}
        uri={feedback.owner.picture}
        style={{marginBottom: 8}}
      />
      <AppText size={22}>{name}</AppText>
      <AppText color="#8083A3" size={14} weight={400}>
        {`${feedback.owner.firstName}src/${feedback.owner.lastName}.com`.toLowerCase()}
      </AppText>
      <View style={styles.section}>
        <AppText size={16}>{`${name}'s rating`}</AppText>
        <AppText color="#8083A3" style={{marginBottom: 18}}>
          See the rating, Cayla Brister got from last reviews
        </AppText>
